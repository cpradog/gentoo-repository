...
src_prepare() {
	...
	eapply ${FILESDIR}/gccemacs-configure.patch
	eapply_user
...
src_install() {
	...
	mv "${ED}"/usr/bin/{gccemacs-${FULL_VERSION}-,}${EMACS_SUFFIX} || die
	mv "${ED}"/usr/share/man/man1/{gccemacs-,}${EMACS_SUFFIX}.1 || die
	mv "${ED}"/usr/share/metainfo/{gccemacs-,}${EMACS_SUFFIX}.appdata.xml || die
	...
	rm "${ED}"/usr/share/gccemacs/site-lisp/subdirs.el
	rm -rf "${ED}"/usr/share/{appdata,applications,icons}
	rm -rf "${ED}/usr/$(get_libdir)"
	rm -rf "${ED}"/var

	# remove unused <version>/site-lisp dir
	rm -rf "${ED}"/usr/share/gccemacs/${FULL_VERSION}/site-lisp

	# remove COPYING file (except for etc/COPYING used by describe-copying)
	rm "${ED}"/usr/share/gccemacs/${FULL_VERSION}/lisp/COPYING
	...
	if use gzip-el; then
		...
		find "${ED}"/usr/share/gccemacs/${FULL_VERSION}/lisp -type f \
	...
	local cdir
	if use source; then
		cdir="/usr/share/gccemacs/${FULL_VERSION}/src"
		insinto "${cdir}"
	...
	local DOC_CONTENTS="You can set the version to be started by
		/usr/bin/gccemacs through the Emacs eselect module, which also
...
pkg_postinst() {
	...
	elif [[ $(readlink "${EROOT}"/usr/bin/gccemacs) = ${EMACS_SUFFIX} ]]; then
